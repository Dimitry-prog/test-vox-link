...
{// TEST 2.1
    ...
    async function sleepOneByOneAsyncAwaited() {
        // Писать код здесь
        const res = await sleep500() + await sleep200() + await sleep100();
        return res;
    }
...
{// TEST 2.3
    ...
    function sleepOneByOne() {
        ...
        for (let i = 0; i < orderedSleeps.length; i++) {
          const currentPromise = orderedSleeps[i];
...
{// TEST 2.4
    ...
    async function sleepParallel() {
        // Писать код здесь
        return Promise.all([sleep100(), sleep200(), sleep500()])
    }
